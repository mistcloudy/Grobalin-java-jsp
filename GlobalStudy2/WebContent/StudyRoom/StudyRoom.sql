...
<!--주훈 DB-->
CREATE TABLE S_JOIN(
    S_JOINCODE number primary key not null,
    S_STUDYCODE number not null,
...
insert into S_JOIN(S_JOINCODE, S_STUDYCODE, M_MEMCODE, SJ_SUBMITYN)VALUES(S_JOIN_seq.nextval, 1, '홍길동', 'Y');
select * from s_join;
select max(S_STUDYCODE) from S_RESIST;
select * from S_RESIST;