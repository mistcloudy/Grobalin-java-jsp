...
public class MyStudyDAO {
	...
	public int getArticleCount3(String code) {
		...
		try {
			con = ConnUtil.getConnection();
			pstmt = con.prepareStatement(" select count(*) from S_RESIST r inner join S_JOIN j on r.S_STUDYCODE =j.S_STUDYCODE and j.SJ_SUBMITYN='N' and r.M_MEMCODE=?"); //  신청번호에서 수락여부 0 인 스터디 번호의 카운트
			pstmt.setString(1, code); 
			rs=pstmt.executeQuery();
			if(rs.next()) {
				x= rs.getInt(1);
