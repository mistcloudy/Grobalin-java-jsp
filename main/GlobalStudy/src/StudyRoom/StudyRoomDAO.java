...
public class StudyRoomDAO {
	...
	public void insertStudy(StudyRoomVO vo) {
		...
		try {
			...
			sql="insert into S_JOIN(S_JOINCODE, S_STUDYCODE, M_MEMCODE, SJ_SUBMITYN, SJ_NAME, SJ_JOINDAY)VALUES(S_JOIN_seq.nextval, ?, ?, 'Y', ?, sysdate)";
			pstmt= con.prepareStatement(sql);
...
public void insertJoin(StudyRoomVO vo) {
		
		Connection con = null;
		PreparedStatement pstmt =null;
		StudyRoomVO room = null;
		String sql = "";
		try {
			con=ConnUtil.getConnection();
			sql="insert into S_JOIN (S_JOINCODE, S_STUDYCODE,  M_MEMCODE, SJ_NAME,  SJ_TITLE, SJ_CONTENT, SJ_SUBMITYN)"
					+ " values(S_JOIN_seq.NEXTVAL,?,?,?,?,?,'N')";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, vo.getStudyCode());
			pstmt.setString(2, vo.getMemCode());
			pstmt.setString(3, vo.getSj_NAME());
			pstmt.setString(4, vo.getSjtitle());
			pstmt.setString(5, vo.getSjcontent());
			pstmt.executeUpdate();
