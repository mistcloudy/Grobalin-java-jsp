<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    String code = request.getParameter("s_STUDYCODE");
    String loginID=(String)session.getAttribute("loginID");
    String code2=(String)session.getAttribute("M_MemCode");
    ...
    count = dbPro.getArticleCount4(code);
  
    if(count>0){
    articleList = dbPro.getArticles5(start, end, code);
    }
                        ...
                        <table class="board_list">
                            ...
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <td class="tit"><%=article.getS_AREA() %></td>
                                    <td><%=article.getSj_NAME() %></td>
                                    <td><%=article.getSj_JOINDAY() %></td>
                                    <td>
                                     <% if (!M_Name.equals(article.getSj_NAME())) {
            	%>
                                   <input type="submit" value="탈퇴" onclick="this.form.action='../스터디관리/멤버탈퇴.jsp'" >
                                     <% 
                }
            	%>
                                   <input type="hidden" name="membercode" value="<%=article.getS_JOINCODE()%>"></td>       
                                </tr>                              
                                	<%} %>
...
if(count>0){
	...
	%>
	<a href="관리멤버.jsp?pageNum=<%=i%>"  class="num">[<%=i%>]</a>
<% 
}
