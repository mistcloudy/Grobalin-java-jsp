<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    count = dbPro.getArticleCount(code);
  
    if(count>0){
    articleList = dbPro.getArticles(start, end, code);
    }
                        ...
                        <table class="board_list">
                            ...
                            <% for(int i=0;i<articleList.size();i++){
                                ...
                                <tr>
                                    ...
                                    <td class="tit"><a href="../스터디정보/등록스터디.jsp?s_STUDYCODE=<%=article.getS_STUDYCODE()%>&memname=<%=article.getM_NAME() %>&pageNum=<%=currentPage%>"><%=article.getS_TITLE() %></a></td>
...
if(count>0){
	...
	if(startPage>pageBlock){
		%>
		<a href="정보메인.jsp?pageNum=<%=startPage-pageBlock%>" class="bt"><</a>
	<%
		}
	for(int i = startPage;i<=endPage;i++){
	%>
	<a href="정보메인.jsp?pageNum=<%=i%>"  class="num">[<%=i%>]</a>
<% 
}
