...
//			pstmt = con.prepareStatement("select * from board order by num desc");
	...
	public void insertArticle( MyStudyVO vo) {
		...
		try {
			con=ConnUtil.getConnection();
			pstmt=con.prepareStatement("select max(num) from S_JOIN");
			rs= pstmt.executeQuery();
			if(rs.next()){
			...
			sql="insert into S_JOIN (S_JOINCODE, S_STUDYCODE,  M_MEMCODE, SJ_DATE, SJ_TITLE, SJ_CONTENT, SJ_SUBMITYN)"
					+ " values(board_seq.nextval,?,?,sysdate,?,?,'N')";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, vo.getS_JOINCODE());
			pstmt.setInt(2, vo.getS_STUDYCODE());
			pstmt.setString(3, vo.getM_MEMCODE());
			pstmt.setTimestamp(4, vo.getSj_DATE());
			pstmt.setString(5, vo.getSj_TITLE());
			pstmt.setString(6, vo.getSj_CONTENT());
			pstmt.setString(7, vo.getSj_SUBMITYN());
			pstmt.executeUpdate();
	...
	public MyStudyVO updateGetArticle(String code) {
		...
		try {
			con = ConnUtil.getConnection();
			pstmt = con.prepareStatement("select * from S_RESIST where code=?");
			pstmt.setString(1, code);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				article = new MyStudyVO();
	...
	public int deleteArticle(String code, String pass) {
		...
		try {
			con=ConnUtil.getConnection();
			pstmt=con.prepareStatement("select pass from S_RESIST where code=?");
			pstmt.setString(1, code);
			rs=pstmt.executeQuery();
			//pstmt.close();
			...
			if(rs.next()) {
				...
				if(dbpasswd.equals(pass)) {
					pstmt= con.prepareStatement("delete from S_RESIST where code=?");
					pstmt.setString(1, code);
					pstmt.executeUpdate();
					result=1;
				}else {
	...
	public int deleteArticle2(String code, String pass) {
		Connection con = null;
		PreparedStatement pstmt =null;
		ResultSet rs=null;
		String dbpasswd ="";
		...
		try {
			con=ConnUtil.getConnection();
			pstmt=con.prepareStatement("select pass from S_JOIN where code=?");
			pstmt.setString(1, code);
			rs=pstmt.executeQuery();
			//pstmt.close();
			...
			if(rs.next()) {
				dbpasswd=rs.getString("pass");
				if(dbpasswd.equals(pass)) {
					pstmt= con.prepareStatement("delete from S_JOIN where code=?");
					pstmt.setString(1, code);
					pstmt.executeUpdate();
					result=1;
				}else {
